    ...
    public class HttpSharedStorageProvider : ISharedAccessStorageProvider
    {
        ...
        private async Task InitializeSessionAsync()
        {
            try
            {
                ...
                {
                    DebugConsole.LogError("HttpSharedStorageProvider: Server connectivity validation failed", false);
                    IsInitialized = false;
                    return;
                }
        ...
        private async Task<T> RetryOperation<T>(Func<Task<T>> operation, string operationName)
        {
            ...
            for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++)
            {
                ...
                {
                    lastException = ex;
                    DebugConsole.LogWarning($"HttpSharedStorageProvider: {operationName} attempt {attempt} failed: {ex.Message}");
