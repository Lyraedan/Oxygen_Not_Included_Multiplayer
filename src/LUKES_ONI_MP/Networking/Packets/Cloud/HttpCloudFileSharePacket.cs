...
namespace ONI_MP.Networking.Packets.Cloud
{
    ...
    /// </summary>
    public class HttpCloudFileSharePacket : IPacket
    {
        ...
        public void OnDispatched()
        {
            ...
            ONI_MP.DebugTools.DebugConsole.Log($"HttpCloudFileSharePacket: Received file share notification - {FileName} ({FileSize} bytes) from {ServerUrl}");
            
            // Auto-download if configured or notify user
            if (ONI_MP.Cloud.SharedAccessStorageManager.Instance.CurrentProvider == "HttpSharedStorage")
            {
